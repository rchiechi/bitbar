#!/usr/local/bin/python3

# <bitbar.title>put.io transfers</bitbar.title>
# <bitbar.version>v1.0</bitbar.version>
# <bitbar.author>Ryan Chiechi</bitbar.author>
# <bitbar.author.github>rchiechi</bitbar.author.github>
# <bitbar.desc>Shows put.io transfers</bitbar.desc>
# <bitbar.dependencies>Python,Requests</bitbar.dependencies>

import requests,json,base64

OAUTH_TOKEN="<YOUR_TOKEN>"
BURL="https://api.put.io/v2"


def humanbytes(B, per=''):
    'Return the given bytes as a human friendly KB, MB, GB, or TB string'
    B = float(B)
    KB = float(1024)
    MB = float(KB ** 2) # 1,048,576
    GB = float(KB ** 3) # 1,073,741,824
    TB = float(KB ** 4) # 1,099,511,627,776

    if B == 0:
       return '0'
    if B < KB:
       return '{0} {1}{2:s}'.format(B,'Byte' if 0 == B > 1 else 'Bytes',per)
    elif KB <= B < MB:
       return '{0:.2f} KB{1:s}'.format(B/KB,per)
    elif MB <= B < GB:
       return '{0:.2f} MB{1:s}'.format(B/MB,per)
    elif GB <= B < TB:
       return '{0:.2f} GB{1:s}'.format(B/GB,per)
    elif TB <= B:
       return '{0:.2f} TB{1:s}'.format(B/TB,per)

def getdir(pid):
    files = []
    r = requests.get(BURL+'/files/list?parent_id=%s&oauth_token=%s' %(pid,OAUTH_TOKEN))
    for f in json.loads(str(r.content,encoding='utf-8'))['files']:
        f['icon'] = str(base64.b64encode(requests.get(f['icon']).content),encoding='utf-8')
        files.append(f)
    return files

print('|image='+str(b'', encoding='utf-8'))

r = requests.get(BURL+'/transfers/list?oauth_token='+OAUTH_TOKEN)
transfers = json.loads(str(r.content,encoding='utf-8'))['transfers']
r = requests.get(BURL+'/account/info?oauth_token='+OAUTH_TOKEN)
info = json.loads(str(r.content,encoding='utf-8'))['info']

print('---')

print('= Transfers (up/down) = | color=gray')
for t in transfers:
    status = t['status']
    if status == 'SEEDING':
        print(':arrows_clockwise: %s | color=green' % t['name'])
    elif status == 'DOWNLOADING':
        print(':arrows_clockwise: %s | color=blue' % t['name'])
    else:
        print(':arrows_clockwise: %s | color=black' % t['name'])
    if t['error_message']:
        print('%s | color=red' % t['error_message'])
    print('--%s: %s / %s | color=black' % (t['status'].capitalize(),humanbytes(t['uploaded']),humanbytes(t['downloaded']) ) )
    print('--Peers: %s / %s | color=black' % (t['peers_getting_from_us'],t['peers_sending_to_us'] ) )
    print('--Speed: %s / %s | color=black' % (humanbytes(t['up_speed'],'/s') ,humanbytes(t['down_speed'],'/s') ))
    if status == 'DOWNLOADING':
        print('--ETA: %0.0f min (%s%%) | color=black' % (t['estimated_time']/60,t['percent_done'] ) )
    else:
        print('--Ratio: %s | color=black' % (t['current_ratio']) )
print('\n---')
for root in getdir(0):
    print('%s (%s) | color=black image=%s' % (root['name'],humanbytes(root['size']),root['icon']) )
    for f in getdir(root['id']):
        print('--%s (%s) | color=black image=%s' % (f['name'],humanbytes(f['size']),f['icon']) )
        for sf in getdir(f['id']):
            print('----%s (%s) | color=black image=%s' % (sf['name'],humanbytes(sf['size']),sf['icon']) )
print('\n---')
print('Disk: %s / %s | color=black' % (humanbytes(info['disk']['used']),humanbytes(info['disk']['size']) ) )
print('Actions')
print('--Go to put.io | color=black href=https://put.io/transfers')
print('--Clean Transfers | refresh=true terminal=false bash=curl param1="--data oauth_token=%s" param2="--url %s/transfers/clean"' %(OAUTH_TOKEN,BURL))
